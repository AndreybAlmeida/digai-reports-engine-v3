...
def _tz_naive(s) -> "pd.Series":
    """
    Converte Series (ou DatetimeIndex) para tz-naive de forma segura.
    Suporta: NaT, tz-aware homogêneo, mix tz-aware+tz-naive.
    """
    if isinstance(s, pd.DatetimeIndex):
        s = s.to_series().reset_index(drop=True)

    idx = s.index if hasattr(s, "index") else range(len(s))
    ...
    # Séries object-dtype podem ter mix de tz-aware/tz-naive — sempre usa elemento-a-elemento
    # para preservar os valores tz-naive (pd.to_datetime coercing-to-UTC silenciosamente perde tz-naive)
    if hasattr(s, "dtype") and s.dtype == object:
        return pd.Series([_safe_v(v) for v in s], index=idx, dtype="datetime64[ns]")

    # Para séries já tipadas como datetime, tenta conversão vetorizada
    try:
        parsed = pd.to_datetime(s, errors="coerce")
        dtype_str = str(parsed.dtype) if hasattr(parsed, "dtype") else ""
        ...
        if isinstance(parsed, pd.DatetimeIndex):
            if parsed.tz is not None:
                return parsed.tz_convert(None).to_series(index=idx)
            return parsed.to_series(index=idx)
    except Exception:
        pass

    # Fallback elemento-a-elemento: sempre funciona com dados heterogêneos
    return pd.Series([_safe_v(v) for v in s], index=idx, dtype="datetime64[ns]")


def _safe_diff_days(a, b) -> float:
    """Diferença em dias entre dois timestamps, lidando com tz-aware/tz-naive."""
    try:
        ta = pd.Timestamp(a)
        tb = pd.Timestamp(b)
        if ta.tzinfo is not None:
            ta = ta.tz_convert(None)
        if tb.tzinfo is not None:
            tb = tb.tz_convert(None)
        return float((tb - ta).days)
    except Exception:
        return np.nan
    ...
    for grupo in [COM_DIGAI, SEM_DIGAI]:
        ...
        if "data_cadastro" in g.columns and "data_final" in g.columns:
            try:
                ...
                contratados_df["sla"] = pd.Series([
                    _safe_diff_days(a, b)
                    for a, b in zip(contratados_df["data_cadastro"], contratados_df["data_final"])
                ], index=contratados_df.index)
        ...
        score_distribuicao      = None   # todos entrevistados (para curva comparativa)
        if grupo == COM_DIGAI:
            for score_col in ("score_ia", "score_inicial_ia", "score"):
                if score_col in contratados_df.columns:
                    ...
                    break
            # Distribuição de todos os Com DigAI (não só contratados)
            for score_col in ("score_ia", "score_inicial_ia", "score"):
...
def calcular_roi(df: pd.DataFrame, params: dict) -> dict:
    ...
    if "data_cadastro" in df.columns and "data_final" in df.columns:
        try:
            ...
            sla_check = pd.Series([
                _safe_diff_days(a, b)
                for a, b in zip(df["data_cadastro"], df["data_final"])
            ], index=df.index)
...
def calcular_origem_candidatos(df: pd.DataFrame) -> list:
    ...
    # Move "Sem DDD" / "Sem Estado" / "Outro" para o final
    grp_validos  = grp[~grp['_estado'].isin(['Sem DDD', 'Sem Estado', 'Outro', ''])]
    grp_invalidos = grp[grp['_estado'].isin(['Sem DDD', 'Sem Estado', 'Outro'])]
    if not grp_validos.empty:
        grp = pd.concat([grp_validos, grp_invalidos])
...
def calcular_tempo_por_etapa(df: pd.DataFrame) -> list[dict]:
    ...
    for (nome, col_atual), (_, col_prox) in etapas_com_next:
        ...
        for grupo in [COM_DIGAI, SEM_DIGAI]:
            ...
            try:
                g["tempo"] = (_tz_naive(g[col_prox]) - _tz_naive(g[col_atual])).dt.days
            except Exception:
                g["tempo"] = pd.Series([
                    _safe_diff_days(a, b)
                    for a, b in zip(g[col_atual], g[col_prox])
                ], index=g.index)
            g = g[g["tempo"] >= 0]
...
def calcular_funil_dinamico(df: pd.DataFrame) -> list[dict]:
    ...
    added_names = {"Cadastro", "Entrevista DigAI"}
    for n in sorted(stage_cols.keys()):
        ...
        # Pega o nome mais comum da etapa (exclui strings vazias/nan)
        if name_col and name_col in df.columns:
            _valid_names = df[name_col].astype(str).str.strip()
            _valid_names = _valid_names[~_valid_names.isin(["", "nan", "None"])]
            top_names = _valid_names.value_counts()
...
def gerar_narrativa(kpis: dict, roi: dict, meta: dict) -> dict:
    ...
    # Assertividade = EIs necessárias por contratação (ratio, não %)
    assertividade_ratio = com.get("assertividade")
    ...
    if roi_val > 0:
        destaques.append(
            ...
            f"(custo IA: R$ {custo_ia:.2f}/candidato vs R$ {custo_ta:.2f} tradicional)"
        )
    if contratados_com > 0 and contratados_sem > 0:
        ...
        destaques.append(
            ...
            f"de {total_geral:,} candidatos no funil"
        )
    ...
    if assertividade_ratio and assertividade_ratio > 0:
        destaques.append(
            f"Assertividade da triagem: 1 contratação a cada {assertividade_ratio:.0f} entrevistas IA"
        )

    oportunidades = []
    if adesao_pct < 50:
        oportunidades.append(
            f"Adesão à EI de {adesao_pct:.1f}% — potencial de crescimento no engajamento dos candidatos"
        )
    if sla_com > 0 and sla_sem > 0 and sla_com > sla_sem + 5:
        oportunidades.append(
            f"SLA do grupo DigAI ({sla_com:.0f} dias) acima do controle ({sla_sem:.0f} dias) "
            "— revisar gargalos pós-triagem"
        )
    ...
    if contratados_sem > 0:
        historia += (
            ...
            f"(taxa de {taxa_sem:.1f}%). "
        )
    if roi_val > 0:
        historia += (
            f"Resultado financeiro: economia de R$ {savings:,.0f} com ROI de {roi_val:.1f}x sobre o investimento."
        )
    else:
...
def calcular_mapa_vagas(df: pd.DataFrame) -> list:
    """
    Retorna top 10 vagas (Com DigAI) com total, contratados, assertividade, adesão e SLA médio.
    - assertividade: % dos Com DigAI que foram contratados
    - adesao: % do total da vaga que é Com DigAI
    - sla_medio: dias médios cadastro→contratação dos contratados
    """
    vaga_col = None
    for c in ("vaga", "vaga_cand", "Nome da vaga", "vaga_digai"):
        if c in df.columns:
    ...
    com = df[df["processo_seletivo"] == COM_DIGAI].copy()
    if com.empty:
        return []

    result = []
    for vaga, grupo in com.groupby(vaga_col, observed=True):
        if not vaga or str(vaga).strip() == "" or pd.isna(vaga):
            continue
        total_com   = len(grupo)
        contratados = int((grupo["status"] == STATUS_CONTRATADO).sum())
        assertividade = round(contratados / total_com * 100, 1) if total_com > 0 else 0.0

        # Adesão: % do total da vaga (com + sem) que está no grupo Com DigAI
        ...
        # SLA médio: dias cadastro → contratação (apenas contratados)
        sla_medio = None
        if contratados > 0 and "data_cadastro" in grupo.columns and "data_contratacao" in grupo.columns:
            hired   = grupo[grupo["status"] == STATUS_CONTRATADO]
            sla_days = (_tz_naive(hired["data_contratacao"]) - _tz_naive(hired["data_cadastro"])).dt.days
